...
public class CBAlignTextView extends TextView {
    ...
    private final static String TAG = "CBAlignTextView";
    ...
    public CBAlignTextView(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, me.codeboy.android.aligntextview.R.styleable.CBAlignTextView);

        isConvert = ta.getBoolean(me.codeboy.android.aligntextview.R.styleable.CBAlignTextView_punctuationConvert, false);
    ...
    //添加的方法，用于高亮显示单词
    public void setChangeText(CharSequence text,ArrayList<String> list,BufferType type){
            oldText = text;
            if (!isAddListener) {
                addLayoutListener();
            }
            process(false);
            style = new SpannableStringBuilder(newText);
            for (String s : list) {
                if (newText.length() != 0) {
                    style.setSpan(new BackgroundColorSpan(Color.YELLOW), newText.toString().indexOf(s), newText.toString().indexOf(s) + s.length(), Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
                    super.setText(style, BufferType.SPANNABLE);
                }
            }
            change = true;
    }
